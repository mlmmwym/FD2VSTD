...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    backbone = resnet50_fpn_backbone(pretrain_path="./save_weights/resnet50.pth", trainable_layers=5)

    model = MaskRCNN(backbone, num_classes=num_classes)

    if load_pretrain_weights:
        # coco weights url: "https://download.pytorch.org/models/maskrcnn_resnet50_fpn_coco-bf2d0c1e.pth"
        weights_dict = torch.load("./save_weights/base1-1200.pth", map_location="cpu")
        for k in list(weights_dict.keys()):
            if ("box_predictor" in k) or ("mask_fcn_logits" in k):
                del weights_dict[k]
        keys_to_delete = [key for key in weights_dict['model'].keys() if key.startswith('roi')]
        for key in keys_to_delete:
            del weights_dict['model'][key]
