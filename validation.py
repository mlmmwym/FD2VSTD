...
def summarize(self, catId=None):
    ...
    stats[2], print_list[2] = _summarize(1, iouThr=.7, maxDets=self.params.maxDets[2])
    stats[3], print_list[3] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
    stats[4], print_list[4] = _summarize(1, iouThr=.8, maxDets=self.params.maxDets[2])
    stats[5], print_list[5] = _summarize(1, iouThr=.85, maxDets=self.params.maxDets[2])
    stats[6], print_list[6] = _summarize(0, maxDets=self.params.maxDets[0])
    stats[7], print_list[7] = _summarize(0, iouThr=.5, maxDets=self.params.maxDets[2])
    stats[8], print_list[8] = _summarize(0, iouThr=.7, maxDets=self.params.maxDets[2])
    stats[9], print_list[9] = _summarize(0, iouThr=.75, maxDets=self.params.maxDets[2])
    stats[10], print_list[10] = _summarize(0, iouThr=.8, maxDets=self.params.maxDets[2])
    stats[11], print_list[11] = _summarize(0, iouThr=.85, maxDets=self.params.maxDets[2])
...
if __name__ == "__main__":
    ...
    # 数据集的根目录
    parser.add_argument('--data-path', default='./datasets/coco2017', help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./save_weights/model_23.pth', type=str, help='training weights')
